...
package name.aiteanu.docmanager.gui.part;
...
//          if (item == null)
//            return;
//          Account account = (Account) item.getData();
//          if(account == null)
//        	  return;
  ...
  private void init() throws RemoteException
  {
    ...
    while (accounts.hasNext())
    {
      ...
      try
      {
        ...
        checked = !uncheckedCache.containsKey(account.getName());
      }
      catch (Exception e)
      {
        Logger.error("unable to determine if job was unchecked",e);
      }
      this.addItem(account, checked);
...
//    b.addButton(i18n.tr("Synchronisierungsoptionen..."),new Options(),null,false,"document-properties.png"); // BUGZILLA 226
  ...
  private void cacheUnchecked()
  {
    try
    {
      ...
      List<Account> selected = getItems(true);
      List<Account> all      = getItems(false);
      for (Account j:all)
      {
  ...
  private class SyncStart implements Action
  {
    ...
    public void handleAction(Object context) throws ApplicationException
    {
      try
      {
        ...
        SynchronizeDocuments sync = new SynchronizeDocuments(selected);
