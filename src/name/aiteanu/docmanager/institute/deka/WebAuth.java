...
public class WebAuth {
	...
	public static WebDriver loginWithSelenium(String responseLogin, String responsePasswort, WebDriver seleniumWebDriver, String optionalOptions, Class<?> externalLogger, Class<?> externalProgressMonitor, Class<?> externalDialogInterface) throws Exception {
		...
		WebDriverWait wait = new WebDriverWait(seleniumWebDriver, 30L);
		WebDriverWait wait10 = new WebDriverWait(seleniumWebDriver, 10L);
		WebDriverWait wait1 = new WebDriverWait(seleniumWebDriver, 1L);
		try {
			LogInfo.invoke(LogInfo, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + InstitutOptions.SHORT_NAME + "-Login aufrufen ... (GET " + InstitutOptions.LOGIN_URL + ")" });
			try {
				seleniumWebDriver.get(InstitutOptions.LOGIN_URL);
				SeleniumUtils.waitForJSandJQueryToLoad(seleniumWebDriver);
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "loginpage: current webdriver hash: " + seleniumWebDriver
			...
			SeleniumUtils.closeCookieLaw(seleniumWebDriver, InstitutOptions.LOGIDENT, InstitutOptions.LONG_NAME, InstitutOptions.LOGO_PATH, cookieDetectPath, cookieAcceptPath, WebUtils.LOADER_CATCHSTRING, WebUtils.LOADER_EXCLUSIONS, WebUtils.LOADER_PATH, WebUtils.LOADER_TEXT, externalLogger);
			try {
				...
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "inputUsername: " + inputUsername });
				WebElement inputPasswd = seleniumWebDriver.findElement(By.cssSelector("aside input.js-password"));
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "inputPassword: " + inputPasswd });
				inputUsername.click();
			...
			*/
			try {
				...
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Depot-Iframe wird angezeigt, Login erfolgreich" });
			} catch (TimeoutException|org.openqa.selenium.NoSuchElementException noBankingApp) {
				LogTrace.invoke(LogTrace, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "... nach dem Login erscheint folgende Seite im XML-Format: \n" + seleniumWebDriver
						.getPageSource() });
				isSelfException = true;
			...
			if (isSelfException == true)
				throw new Exception(error.getMessage()); 
			LogError.invoke(LogError, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Fehlermeldung/Exception des Systems:\n" + 
					ExceptionUtils.getStackTrace(error) });
	...
	public static void logoutWithSelenium(WebDriver seleniumWebDriver, String optionalOptions, Class<?> externalLogger, Class<?> externalProgressMonitor, Class<?> externalDialogInterface) throws Exception {
		...
		try {
			LogInfo.invoke(LogInfo, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Abmelden aufrufen (" + InstitutOptions.LOGOUT_URL + ")" });
			try {
				seleniumWebDriver.get(InstitutOptions.LOGOUT_URL);
				SeleniumUtils.waitForJSandJQueryToLoad(seleniumWebDriver);
				Thread.sleep(1000L);
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "loginpage: current webdriver hash: " + seleniumWebDriver
						.getWindowHandle().hashCode() });
			...
			LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "postLogoutPage: " + seleniumWebDriver });
			...
			try {
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Gefundene Logout-Best" + wait
				...
				LogTrace.invoke(LogTrace, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Seite nach Logout-Aufruf unbekannt; Code zur Analyse:\n" + postLogoutPage });
				isSelfException = true;
				throw new Exception("Seite nach Logout-Aufruf unbekannt; Bitte dem Entwickler im Forum melden!");
			} catch (Exception error) {
				throw new Exception("WebDriver-Fehler: " + ExceptionUtils.getStackTrace(error));
			} 
			LogInfo.invoke(LogInfo, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Logout bei der '" + InstitutOptions.LONG_NAME + "' war erfolgreich" });
		} catch (Exception error) {
			if (isSelfException == true)
				throw new Exception(error.getMessage()); 
			LogError.invoke(LogError, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "WebLogout fehlerhaft! Stacktrace:\n" + 
					ExceptionUtils.getStackTrace(error) });
