...
//			SeleniumUtils.closeCookieLaw(seleniumWebDriver, InstituteOptionsBaader.LOGIDENT, InstituteOptionsBaader.LONG_NAME, InstituteOptionsBaader.LOGO_PATH, cookieDetectPath, cookieAcceptPath, WebUtils.LOADER_CATCHSTRING, WebUtils.LOADER_EXCLUSIONS, WebUtils.LOADER_PATH, WebUtils.LOADER_TEXT, externalLogger);
			...
			do {
				LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "PrÃ¼fe auf Verfahren der Zwei-Faktor-Authentifizierung ..." });
				isChipTan = false;
				isChipTanQR = false;
				isAppTan = false;
				isAppConfirm = false;
				...
				if (!haveFound)
					try {
						...
						LogDebug.invoke(LogDebug, new Object[] { InstitutOptions.LOGIDENT + getLogMethod + "Sicherheitsverfahren 'TAN2go' verlangt wohl keine Wahl des EngerÃ¤ts"});
					}  
				if (!haveFound)
					try {
				...
				if (!haveFound) {
					...
					try {
						...
						MonitorLog.invoke(MonitorLog, new Object[] { "Info-Warnung: chipTan optisch (Flickercode) ist als Verfahren gesetzt, wird aber NICHT unterstÃ¼tzt."});
